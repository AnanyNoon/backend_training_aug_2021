...
class Context(ctxutil.ContextBase):
    ...
    def push(self):
        ...
        if self.isolation_level:
            self.conn = libbaz.engine.connect().execution_options(isolation_level=self.isolation_level)
        else:
            self.conn = libbaz.engine.connect()
    ...
    def pop(self, exc_type=None, exc_value=None, tb=None):
        ...
        if isinstance(exc_value, AssertionError):
            raise libbaz.DomainException(exc_value.args[0] if exc_value.args else 'error',
